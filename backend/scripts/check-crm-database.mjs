...
if (process.env.CRM_DB_CHECK_CONFIRM !== 'CHECK_CRM_002') {
  ...
  try {
    const expectedTables = ['crm_cases', 'crm_dealers', 'crm_dealer_contacts', 'crm_case_dealers', 'crm_case_events', 'crm_aftercare_tasks', 'crm_product_events'];
    ...
    const required = { crm_cases: ['stage', 'lead_id', 'purchase_request_id'], crm_dealers: ['legal_name', 'status', 'data_processing_status'], crm_dealer_contacts: ['dealer_id', 'contact_name', 'email'], crm_case_events: ['case_id', 'to_stage'], crm_aftercare_tasks: ['case_id', 'task_type', 'status'], crm_product_events: ['event_id', 'event_type', 'created_at'] };
