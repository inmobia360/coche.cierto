...
const gates = [
  ['Migraciones CRM disponibles', fs.existsSync(path.join(backend, 'migrations', '002_crm_lifecycle_up.sql')) && fs.existsSync(path.join(backend, 'migrations', '003_crm_observability_up.sql')), 'archivos locales'],
  ...
  ['Confirmación de migración presente', process.env.CRM_MIGRATION_CONFIRM === 'APPLY_CRM_002' || process.env.CRM_MIGRATION_CONFIRM === 'APPLY_CRM_003', 'no se ejecuta desde este comprobador'],
